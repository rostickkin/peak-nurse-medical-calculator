...
const filePath = path.join(__dirname, "../src/data/research/drugs-17-20.json");
...
function validate() {
  ...
  console.log(`Number of drugs present: ${data.length}`);
  ...
  if (uniqueIds.size !== ids.length) {
    errors.push("Duplicate drug IDs found");
  }

  // Check exactly 4 drugs
  if (data.length !== 4) {
    errors.push(`Expected 4 drugs, found ${data.length}`);
  }
